...
class Camera{
  ...
  float distToLight(PVector v){
    float lightacumulation = 0;
    int lightamount = 0;
    for(int i = 0; i < lights.length; i++){
      PVector d = PVector.sub(lights[i].pos, v);
      d.normalize();
      Ray r = new Ray(v,d);
      PVector p = r.intersects(objects[0]);
    //  println(v + " " + p);
   ...
   //   if(v.dist(lights[0].pos) < p.dist(lights[0].pos))    //if distance current point - light is smaller than intersectPoint - light --> current point is exposed to light
        lightacumulation += v.dist(lights[i].pos);
        lightamount ++;
        println(lightamount + " " + lightacumulation + " " + v.dist(lights[i].pos));
      }
    }
    return lightamount == 0? -1 : lightacumulation / lightamount;
  }
