class Cell {
  ...
  void roundEdges(){
    for(int i = 0; i < eds.size(); i++){
      eds.get(i).points[0].x = round(eds.get(i).points[0].x);
      eds.get(i).points[0].y = round(eds.get(i).points[0].y);
      eds.get(i).points[1].x = round(eds.get(i).points[1].x);
      eds.get(i).points[1].y = round(eds.get(i).points[1].y);
    }
  }
  ...
  void sortEdges(int index) {  
    ...
    for(int i = index + 1; i < eds.size(); i++){
      ...
      if(curEdge.points[1].equals(eds.get(i).points[0]) || curEdge.points[1].equals(eds.get(i).points[1])){
        if(curEdge.points[1].equals(eds.get(i).points[1])){  //switch points of Edge
